...
uint8_t NBIoT_UDP::createSocket(String serverIP,int port,bool receiveCtrl,bool ipv6)
{
	ser_nbiot.flush();
	ser_nbiot.flush();
	ser_nbiot.printf("AT+NSOCR=DGRAM,17,%d,%d",port,receiveCtrl);
	if(ipv6)
...
void NBIoT_UDP::setServer(String serverIP,int port)
{
	serverIP_ = serverIP;
	port_ = port;
}
bool NBIoT_UDP::write(uint8_t *data,size_t length)
{
	static uint8_t seq=1;
	if(socket_==0xFF)
		return false;
	
    ser_nbiot.printf("AT+NSOST=%d,",socket_);
	ser_nbiot.print(serverIP_);
	ser_nbiot.printf(",%d,%d,",port_,length);
...
bool NBIoT_UDP::print(String data)
{
...
size_t NBIoT_UDP::printf(const char *format, ...)
{
    char loc_buf[64];
    char * temp = loc_buf;
    va_list arg;
...
size_t NBIoT_UDP::udpAvailable(unsigned long tout)
{
...
int NBIoT_UDP::strToHex(char *hstr)
{
	return (int) strtol(hstr, 0, 16);
}
/*bool NBIoT_UDP::connected()
{
	if(socket_==0xFF)
		return false;
	return true;
