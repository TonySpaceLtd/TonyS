...
TonyQRCode::TonyQRCode()
{
	xHandle = NULL;
}
void TonyQRCode::init(TonyCam *cam)
{
	cam_ = *cam;
	ready_ = xSemaphoreCreateBinary();
}
qrResoult TonyQRCode::recognition(dl_matrix3du_t *image888)
{	
disableCore0WDT();
	qrResoult res;
	res.status = false;
...
String TonyQRCode::dataType(uint8_t type)
{
	 switch (type) {
